...
router.post("/register",async(req,res)=>{
    try {
        ...
        if(findUser.length>=1){
            ...
            const link=`https://shorturlproject.netlify.app/activation/${registeringUser[0]._id}`
            //composing mail
            const composingMail={
        ...
        if(checkUser.length===0){
            ...
            if(validatingPassword){
                ...
                if(checkUser[0].status==="Active"){
                    ...
                    const link=`https://shorturlproject.netlify.app/activation/${checkUser[0]._id}`
                    //composing mail
                    const composingMail={
                        from:"fullstackpurpose@gmail.com",
...
router.post("/shortUrl",isAuthorized,async(req,res)=>{
    try {
        ...
        const link=`https://shorturlproject.netlify.app/new/${randomString}`
        //adding short url 
